package com.fatkhur.intentfatkhur
...
class MainActivity : AppCompatActivity(), View.OnClickListener{
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnResultFromActivity = findViewById(R.id.tv_result)
        ...
        if (v != null) {
            when (v.id) {
                R.id.btn_move_activity -> run {
                    val intent = Intent(this, btnMoveActivity::class.java)
                    startActivity(intent)
                }
                R.id.btn_move_activity_data -> run {
                    val intent = Intent(this,MoveWithDataActivity::class.java)
                    val bundle = Bundle()
                    bundle.putString("Nama","Fatkhur")
                    bundle.putString("Alamat","Pasuruan")
                    intent.putExtras(bundle)
                    startActivity(intent)
                }
                R.id.btn_dial_number -> run {
                    var dialNumber = "085786644169"
